...
def csv():
    ...
    if request.method == 'POST':
        ...
        if 'input' in request.form:
            ...
            c.execute('''CREATE TABLE sales(name text, date text, price real)''')
            # CSVをpandasで取得する
            df = pd.read_csv('Python_Flask/app/static/csv/list.csv')
            # INSERT文作成と実行
            for i in range(len(df.index)):
                c.execute("INSERT INTO sales VALUES ('{name}', '{date}', '{price}')" .format(
        ...
        # 画面から入力した値を更新する
            ...
            try:
                ...
                if request.form['name'] == '' or request.form['date'] == '' or request.form['price'] == '': 
                    raise ValueError("全ての項目に入力してください。")
                # 入力した値で更新する
                c.execute("UPDATE sales SET date = '{date}',price = '{price}' where name = '{name}'" .format(
                        name=request.form['name'], date=request.form['date'], price=request.form['price']))
                # 結果を保存（コミット）する
                conn.commit()
